...
- **Site resolution**: `HostFilter` resolves domain to Site from `X-Site-Host` (falling back to `Host`) on every blog request, and reads the mount path from `X-Forwarded-Prefix`.
- **Two hosting modes**: a managed subdomain (`myblog.writeinone.com` — created `VERIFIED`, always `prefix = ''`) or the user's own domain behind their reverse proxy (`example.com/blog` — needs `/_verify` DNS verification). Never add per-user subdomains of *their* domain; the gateway cannot provision certs for those.
- **Home-domain checks go through `SubdomainProperties.isHomeDomain()`**. Never compare a host to `"writeinone.com"` inline — the base domain is configurable and `www` counts too.
...
- **Admin UI**: static HTML in `src/main/resources/static/admin/` + JS in `static/js/`, shared `/css/admin.css`. Served by `AdminHandler.serve`, **not** Thymeleaf — `templates/admin/` is empty.
