...
- **JUnit 5 + MockK + Testcontainers** — testing (note: reactive testing with WebFlux has more friction, use
  `StepVerifier`)
...
- Site layout (nav, footer, buttons) is configured via a JSONB `config` column keyed by language — the app controls HTML
  structure, users control content and appearance
...
- **Admin UI** — Thymeleaf + vanilla JS for Markdown live preview (using marked.js via CDN). Everything lives in one
  project, no separate frontend repo, no CORS to deal with.
...
- Image uploads or URLs only? (S3/R2 vs just storing cover image URLs) — Phase 1 uses URLs only; image uploads would be
  a nice addition in Phase 2